class FrogPhysics < Component
  MOVING_DELAY = 150 # Slow down moving
  attr_accessor :speed, :in_collision, :collides_with, :moved_at
  ...
  # 12 11   8   7
  def box
    # TODO - Update box to match frog shape
    w = box_width / 2 - 1
    h = box_height / 2 - 1
    tw = 8 # track width
    fd = 8 # front depth
  ...
  def update
    if object.moving && !object.life.dead?
      accelerate
    else
      decelerate
    end
